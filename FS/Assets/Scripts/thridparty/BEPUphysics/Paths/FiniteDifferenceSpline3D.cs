...
namespace BEPUphysics.Paths
{
    ...
    public class FiniteDifferenceSpline3D : HermiteCurve3D
    {
        ...
        protected override void ComputeTangents()
        {
            ...
            Vector3.Multiply(ref tangentA, Fix64Utils.PointFive / (ControlPoints[1].Time - ControlPoints[0].Time), out tangentA);
            ...
            for (int i = 1; i < ControlPoints.Count - 1; i++)
            {
                ...
                Vector3.Multiply(ref tangentA, Fix64Utils.PointFive / (ControlPoints[i + 1].Time - ControlPoints[i].Time), out tangentA);
                Vector3.Multiply(ref tangentB, Fix64Utils.PointFive / (ControlPoints[i].Time - ControlPoints[i - 1].Time), out tangentB);
                Vector3.Add(ref tangentA, ref tangentB, out tangentA);
                tangents.Add(tangentA);
            }
            ...
            Vector3.Multiply(ref tangentB, Fix64Utils.PointFive / (ControlPoints[currentIndex].Time - ControlPoints[previousIndex].Time), out tangentB);
            //Vector3.Add(ref tangentA, ref tangentB, out tangentA);
